...
或者使用网络安装：net install lsaplot, from(https://raw.githubusercontent.com/hurilen/lsaplot/main/) replace
...
$$
Y_{it} = μ_i + λ_t + Σ_{k=underline{k}}^{bar{k}} β_k · {D_{it}}^k + δ X_{it} + ε_{it}
$$
...
| 选项 | 描述 | 注意事项 |
| :--- | :--- | :--- |
| **`Treat(var)`** | 表示处理组首次受政策冲击的gvar变量。 | 控制组该变量需为空值。**不要填0/1虚拟变量**。 |
...
| `nograph` | - | - | 仅跑回归，不画图。 |
| `name(#)` | - | - | 命名图表名字，便于后续使用`graph combine`合并图表。 |
